...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1, flag= False):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.threads[i] = Thread(target=self.update, args=([i, cap, s, flag]), daemon=True)
    ...
    def update(self, i, cap, stream, flag):
        # Read stream `i` frames in daemon thread
        n, f = 0, self.frames[i]  # frame number, frame array
        print("stream_loaders >>> update 함수 중 : ", flag)

        ###### 안에 갇힘
        while cap.isOpened() and n < f:
            print("stream_loaders >>> while cap.isOpened() and n < f 일 때, : ", flag)
            n += 1
