...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            T = [
                A.RandomResizedCrop(height=640, width=640, scale=(0.8, 1.0), ratio=(0.9, 1.11), p=0.0),
                A.Blur(p=0.01),
                A.MedianBlur(p=0.01),
                A.ToGray(p=0.01),
                A.CLAHE(p=0.01), # 이미지의 대비(contrast)를 향상시키는 기법
                ...
                A.ShiftScaleRotate(shift_limit= 0.05, scale_limit= 0.06, rotate_limit=20, p= 0.33),

                A.ColorJitter(p=0.5), # 이미지의 색상을 바꿈
                A.RGBShift(r_shift_limit=30, g_shift_limit=30, b_shift_limit=30, p=0.33), # 입력 RGB 이미지의 색상을 무작위로 변환하여 색감 추가
                A.Posterize(p=0.1), # 각 이미지 채널의 bit를 감소시킴
                # A.ChannelShuffle(p=0.33), # 이미지의 RGB 채널의 순서를 무작위로 바꿈
