...
file_path = "./raw_test_data"   # 위치 자신의 파일경로에 맞게 변경 필요
image_list = glob.glob(os.path.join(file_path, "*","*"))
folder_list = glob.glob(os.path.join(file_path, "*"))
for create_folder in folder_list:
    create_folder = create_folder.replace("raw_test_data", "resize_test_data") 
    os.makedirs(create_folder, exist_ok=True) 

for item_path in image_list:
    try:
        newfile_path = item_path.replace("raw_test_data", "resize_test_data") # raw_data -> resize_data
