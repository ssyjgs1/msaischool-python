...
raw_path = "./test_data"        # raw_data는 그대로 해주시고 raw_data 전 경로 변경 필요 !!!
...
for age in switch_int_age:
    ...
    if (age >= TWENTY) & (age < THIRTY):
        ...
        image_list = sorted(glob.glob(os.path.join(prev_path,str(age), FEMALE, '*.jpg')), key=os.path.getsize, reverse=True)[CUT+1:CUT+11]  
        try:
        ...
        image_list = sorted(glob.glob(os.path.join(prev_path,str(age), MALE, '*.jpg')), key=os.path.getsize, reverse=True)[CUT+1:CUT+11]  
        try:
            for image_path in image_list:
