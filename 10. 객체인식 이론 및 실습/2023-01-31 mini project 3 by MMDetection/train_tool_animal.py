...
class AnimalDataset(CocoDataset) : # 상속받는 건 CocoDataset | json 파일에 적힌 label(=class 정보)대로 입력하면 됨
...
cfg.checkpoint_config.interval = 88 # 모델 파일 중간 저장 몇 번에 한 번씩 할래?

cfg.runner.max_epochs = 88 # epoch 설정 | 기본값 : 8(GPU 갯수) x 12 = 96, 지금은 GPU 1개라 저렇게 설정
cfg.seed = 777 # 고정된 seed값 지정
cfg.data.samples_per_gpu = 8 # single GPU에서 통용되는 값 | batch_size라고 이해하면 됨
