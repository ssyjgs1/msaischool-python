import albumentations as A # pip install albumentations==1.0.3
import cv2
import numpy as np
...
"""loss, iteration 업데이트 하는 부분 | 각 손실값에 대해 평균도 구하고"""
class Average :
    def __init__(self) :
    ...
    @property # 읽기 전용 - 붙어있는 줄 부분만 해당함. 여기서는 def value 부분만 해당됨
    def value(self) :
...
def collate_fn(batch) :
    """to handle the data loading as different images may have different number of objets and to handle varying size tensors as well."""
    # :param batch:
